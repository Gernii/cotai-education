<script lang="ts">
	...
	const onCourseAccess = async () => {
		...
		try {
			...
			const data = (await response.json()) as LandingPageCourseCheckStatus.Response;
	...
	const onCourseForceAccess = async () => {
		if (userIdAccessed === undefined) {
			toast.error('Something went wrong. no user found, please try again.');
			return;
		}
		...
		try {
			...
			const data = (await response.json()) as LandingPageCourseForceAccess.Response;
...
<li>
	...
	<dialog class="modal" bind:this={dialogRef}>
		<div class="modal-box">
			...
			<h3 class="text-lg font-bold">Checking access to course: {title}</h3>
			{#if isCourseAccessed}
				<div class="pt-4">
					<p class="text-gray-500">
						<span class="font-semibold text-error">{usernameAccessed}</span>
				...
				<div class="modal-action">
					...
					<button class="btn btn-error" on:click={onCourseForceAccess}> Force Access </button>
				</div>
			{/if}
		</div>
